 ...
 */
 ...
 int main(int argc, char** argv)
 {
	 ...
	 while (input[0] != 'q')
	 {
		 ...
		 // Create an array to store 5 cards
		 Card cardArray[5];
		 
		 // Fill the array with 5 cards
		 for (int i = 0; i < 5; i++) {
			 cardArray[i] = getCard();
		 }
		 
		 // Variables to count suits
		 int clubCount = 0;
		 int diamondCount = 0;
		 int heartCount = 0;
		 int spadeCount = 0;
		 ...
		 // Print the counts in required format
		 printf("C %d D %d H %d S %d\n", clubCount, diamondCount, heartCount, spadeCount);
